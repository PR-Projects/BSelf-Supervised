...
def main(args):
    ...
    save_dir_epoch = save_dir / 'ckpts' / args.ds
    save_dir_mcmc = save_dir / 'samples'/ args.ds / f'{args.optimizer}_{args.exp}' 
    ...
    HPS = vars(args)    
    with open(save_dir /'params'/'pretrain'/args.ds/f'{args.exp}_{args.model_type}param.json','w') as file:    
    ...
    online_network = network('online',backbone,args.mlp_hid_size,args.proj_size).to(device)
    target_network = copy.deepcopy(network('target',backbone,args.mlp_hid_size,args.proj_size)).to(device)
    ...
    for epoch in range(args.num_epochs):
        ...
        for phase in ['train','val']:
            ...
            for (img1,img2),_ in dataloader:
                ...
                loss = Train(online_network,target_network,optimizer,img1,img2,phase,N_train,batch_idx,cycle_batch_length,epoch)
