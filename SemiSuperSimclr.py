...
def single_finetune(exp,model,optimizer,criterion,train_loader,val_loader,epochs,\
    ...
    crv_dir = save_dir_fine.parents[2] / 'simclr_lr_curves' / 'finetune' / ds
...
parser.add_argument('--exp',type=int,default=1011,
                        help='ID of this expriment!')

parser.add_argument('--ds_pr',type=str,default='stl10',choices=('cifar10','cifar100','stl10','tinyimagenet','imagenet10'),
                        help='Dataset that we pretrained the model on that!')

parser.add_argument('--ds_ft',type=str,default='stl10', 
                    choices=('cifar100','cifar10','stl10','imagenet10','tinyimagenet'),
                        help='Dataset for semisupervised learning or transfer learning!')

# Fine-tunning parameters
parser.add_argument('--fine',type=bool, default=True,
...
parser.add_argument('--burn_in',type=int,default=0,
                        help='The burn in epochs in mcmc!')
...
# Optimizer & lr
parser.add_argument('--opt',type=str,default='sgd',choices=('adam','sgd','sghm'),
...
parser.add_argument('--wd',type=float, default=0,
                       help='weight decay for fine tunning!')

parser.add_argument('--num_epochs',type=int, default=50,
                       help='The number of epoch for fine tunning!')
...
parser.add_argument('--model_type',type=str, default='', choices=('','b','c','cb'),
                       help='If we want to finetune simclr, bsimclr, csimclr, cbsimclr!')

parser.add_argument('--model_depth',type=str,choices=('res18','res50','res100'),default='res18',
                        help='Model that we used as feature extractor in pretraining!')
...
parser.add_argument('--batch_size',type=int, default=80,
                       help='The number of batch size for finetuning')
 
parser.add_argument('--exp_split',nargs="+", default=[100],
                       help='The split of train set that we want to use for fine tunning!')

# Evaluation parameters 
parser.add_argument('--pr_ens',type=bool, default=True,
                       help='If we want to save pr ensemble or not!')

parser.add_argument('--eval',type=bool, default=True,
                       help='If we want to evaluate our model or not!')

parser.add_argument('--ig_sam',type=int, default=0,
...
def semi_supervised(args):
    ...
    load_dir  = save_dir / 'simclr_ckpts' 
    split_dir = save_dir / 'split_data' 
    exp_dir = save_dir / 'simclr_ckpts'/'samples'/ args.ds_pr
    param_dir = save_dir / 'simclr_params' / 'finetune' / args.ds_ft / f'exp_{args.exp}_opt_{args.opt}_pr_{args.ds_pr}'
    
    ## directory for saving the results 
    result_dir_exp = save_dir / 'simclr_results' / f'{args.ds_ft}_{args.exp_split[0]}'/ f'{args.exp}_pr_{args.ds_pr}'
    ...
    # loadig hyperparameters of pretrained model
    with open(save_dir /'simclr_params'/'pretrain'/args.ds_pr/f'{args.exp}_{args.model_type}param.json') as file:    
        HPP = json.load(file)
        in_size = HPP['in_size']
    ...
    # loading ckt list
    ckt_list = os.listdir(os.path.join(load_dir,'samples/%s/%s_%d'%(config['ds_pr'],config['opt'],config['exp'])))
    ckt_epochs = torch.load(os.path.join(load_dir,'samples/%s/%s_%d_epochs.pt'%\
    ...
    for train_split in config['exp_split']:
        ...
        if config['fine']:
            ...
            for i in range(config['burn_in'],n_ckts_tot):
                ...
                net = network(backbone,config['mlp_hid_size'],config['proj_size']).to(device)
                ...
                save_dir_fine = save_dir / 'simclr_ckpts' / 'finetune' / config['ds_ft']
        ...
        if config['write_exp']:
            ...
            # ./results
            csv_path = Path(os.path.join(save_dir,'simclr_results/%s_%d/run_sweeps_test.csv'%(config['ds_ft'],config['exp_split'][0])))
